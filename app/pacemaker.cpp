...
boost::shared_ptr<rst::timing::Timestamp>
create_rsb_time(uint timestamp){
...
std_msgs::UInt64
create_ros_time(uint timestamp){
...
void sleep_until(uint last_time_micros, uint sum_wait_micros){
  ...
  std::cout << "...needed: " << waited << " ...waiting for " << sum_wait_micros - waited << std::endl;
...
int main(int argc, char **argv){
  ...
  std::string description = "This application periodically sends its current time via ros and rsb.";
  ...
  auto ros_informer = pontoon::io::ros::Informer<std_msgs::UInt64>(topic,"pacemaker");

  std::cout << "Ready. Sending the current time every " << delta << " milliseconds" << std::endl;
